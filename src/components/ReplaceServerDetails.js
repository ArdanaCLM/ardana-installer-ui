...
import {
  genUID, getAvailableServerIds, maskPassword, getModelMacAddresses, getModelIPMIAddresses  }
...
class ReplaceServerDetails extends Component {
  ...
  componentWillMount() {
    if(this.props.data) {
      ...
      this.existMacAddresses = getModelMacAddresses(this.props.model);
      this.existIPMIAddresses = getModelIPMIAddresses(this.props.model);
    }
  ...
  renderInput(name, type, isRequired, title, validate) {
    let theProps = {};
    if(name === 'mac-addr') {
      theProps['exist_mac_addresses'] = this.existMacAddresses;
    }
    if(name === 'ilo-ip') {
      theProps['exist_ip_addresses'] = this.existIPMIAddresses;
    }
    return (
      <InputLine
