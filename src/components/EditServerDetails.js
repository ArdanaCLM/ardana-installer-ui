...
class EditServerDetails extends Component {
  constructor(props) {
    ...
    this.originalId = this.data.id; //saved the original id
    this.state = {
      isFormValid: false,
  ...
  handleDone = () => {
    this.props.doneAction(this.data, this.originalId);
  }
  ...
  renderId() {
    if (this.props.readonlyId) {
      return this.renderTextLine('server.id.prompt', this.data.id);
    }
    else {
      return this.renderInput('id', 'text', true, 'server.id.prompt', UniqueIdValidator);
    }
  }
