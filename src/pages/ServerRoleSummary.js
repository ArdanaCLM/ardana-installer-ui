...
class ServerRoleSummary extends BaseWizardPage {
  ...
  componentWillMount() {
    fetchJson('/api/v1/server?source=sm,ov')
      .then((rawServerData) => {
        if(rawServerData) {
          this.servers.auto = rawServerData;
        }
      });
    ...
    fetchJson('/api/v1/server?source=manual')
      .then((responseData) => {
        if (responseData.length > 0) {
          this.servers.manual =  responseData;
        }
      });
  }
  ...
  updateServerForEditServer = (server, originalId) => {
    for (let list of ['auto', 'manual']) {
      let idx = this.servers[list].findIndex(s => originalId === s.id);
      if (idx >= 0) {
        let updated_server;
        let tempList = this.servers[list].slice();
  ...
  renderCollapsibleTable() {
    ...
    return (
      <CollapsibleTable
        ...
        autoServers={this.autoServers} manualServers={this.manualServers}
