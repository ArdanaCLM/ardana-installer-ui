...
class AssignServerRoles extends BaseWizardPage {
  ...
  checkUpdateServerDataToMatchModel = (serverData, modelServers) => {
    let modelServer = modelServers.find(server => {
      return server.id === serverData.id;
    });
  ...
  renderEditServerDetailsModal() {
    ...
    if(this.state.activeRowData) {
      ...
      let autoServer = this.state.rawDiscoveredServers.find(
        server => {
          // don't allow to change id on auto discovered server
          // for now
          if(server.id === this.state.activeRowData.id) {
            return true;
          }
          else {
            return false;
