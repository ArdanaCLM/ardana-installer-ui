...
import { getPages } from './utils/DynamicPagesFinder.js';
...
class InstallWizard extends Component {
  ...
  constructor(props)
  {
    ...
    this.state = {
      ...
      // TODO how to deal wtih install password
  ...
  loadUpdateProgress = (responseData) => {
    ...
    if(responseData && responseData.steps) {
      ...
      let pages =
        getPages(responseData.currentMenuName, responseData.currentOperation, steps);
  ...
  // this method was originally in componentDidMount
  // in order to reuse the same code for update, kind of
  // move some process out to make it suitable for both install
  // and update. The goal is to keep the install logic intact
  // at this point
  ...
  componentWillMount = () => {
    // call load function here for update since
    // there are issues when call load function in componentDidMount
    // have to call it here TODO
    if(this.IS_UPDATE) {
      this.loadModelandProgress();
    }
  }

  componentDidMount = () => {
    // keep the install load function as it is called
